...
from price_index.price_index_data_config import (username, dbname, password, all_prod_id,
                   global_zone_id, chain_id, queries, product_data, location_data, cal_data)
...
def prod_lookup(product_name = None, product_id = None, product_level = None,
    ...
    hint_match = extractOne(hint,
                            set(product_data['types']['product-type'].values) | {'department', 'section'},
                            scorer = QRatio,
                            score_cutoff = 50)
    hint_match = hint_match[0] if hint_match else None
    if hint_match == 'department':
        product_level = 5
    elif hint_match == 'section':
        product_level = 3
    elif hint_match in product_data['types']['product-type'].values:
        product_level = product_data['types'].loc[product_data['types']['product-type'] == hint_match,
...
def loc_lookup(location_name = None, location_id = None, location_level = None,
               exact_match = True, fuzzy_match = True, contains = True, active = 'Y', n = 1):
    ...
    if location_level in [1, 2, 3, 4, 5, 6]:
